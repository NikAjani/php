...
class Adapter {
    ...
    function fetchAll($tableName) {
        return $this->query("SELECT * FROM `$tableName`")->fetch_all(MYSQLI_ASSOC);
    }

    function fetchRow($tableName, $whereCondition = '1') {
        return $this->query("SELECT * FROM `$tableName` WHERE $whereCondition")->fetch_all(MYSQLI_ASSOC)[0];
    }
